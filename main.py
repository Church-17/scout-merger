...
from tkinter import constants as tk_const, filedialog, Tk, Menu
...
class ScoutMergerGUI:
    ...
    def setup_window(self):
        ...
        self.recap_frame.grid(row=1, column=0, rowspan=2, padx=10, pady=8, sticky=tk_const.NW)
        ...
        self.header_frame.grid(row=0, column=0, sticky=tk_const.NW)
        self.data_frame = Frame(self.recap_frame)
        self.data_frame.grid(row=0, column=1, sticky=tk_const.NW)
        ...
        for row in range(len(self.header)):
            ...
            self.recap_values[row].grid(row=row, column=0, padx=5, sticky=tk_const.W)
        
        self.model_frame = Frame(self.window)
        self.model_frame.grid(row=1, column=1, padx=10, pady=8, sticky=tk_const.NE)
        ...
        self.upload_button.grid(row=0, column=1, padx=5, pady=3, sticky=tk_const.W)
        ...
        self.notify_frame.grid(row=2, column=0, padx=20, pady=10, sticky=tk_const.SW)
        self.notify_label = Label(self.notify_frame, font=("Segoe UI", 12))
        self.notify_label.grid(row=0, column=0, pady=5)

        self.button_frame = Frame(self.window)
        self.button_frame.grid(row=2, column=1, padx=10, pady=12, sticky=tk_const.SE)
        self.save_button = Button(self.button_frame, text="Salva", state=tk_const.DISABLED, command=self.save)
    ...
    def try_enable_save(self):
        if self.loc_picked and self.model_picked:
            self.save_button.config(state=tk_const.ACTIVE)
    ...
    def check_input(self, event):
        self.loc_combo.config(values=list(self.loc_info[self.loc_info[self.header[0]].str.lower().str.contains(self.loc_combo.get().lower())][self.header[0]]))
        self.loc_combo.event_generate('<Down>')
        self.window.after(1, self.loc_combo.winfo_children)

    def update_recap(self, event):
        if self.loc_combo.current() < 0:
            return
        self.loc_dict = self.loc_info[self.loc_info[self.header[0]] == self.loc_combo.get()].iloc[0]
